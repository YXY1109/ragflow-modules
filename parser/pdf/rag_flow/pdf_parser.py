...
from parser.pdf.rag_flow.vision.layout_recognizer import LayoutRecognizer
from parser.pdf.rag_flow.vision.ocr import OCR
from parser.pdf.rag_flow.vision.recognizer import Recognizer
from parser.pdf.rag_flow.vision.table_structure_recognizer import TableStructureRecognizer
...
class RAGFlowPdfParser:
    def __init__(self, **kwargs):
        ...
        model_dir = r"D:\PycharmProjects\ragflow\rag\res\deepdoc"
    ...
    def _updown_concat_features(self, up, down):
        ...
        tks_all = up["text"][-LEN:].strip() + (
            " " if re.match(r"[a-zA-Z0-9]+", up["text"][-1] + down["text"][0]) else "") + down["text"][:LEN].strip()
        ...
        fea = [
            ...
            up["text"].strip()[-2:] == down["text"].strip()[-2:] if len(up["text"].strip()) > 1 and len(
    ...
    def sort_X_by_page(arr, threshold):
        ...
        for i in range(len(arr) - 1):
            for j in range(i, -1, -1):
                ...
                if abs(arr[j + 1]["x0"] - arr[j]["x0"]) < threshold and arr[j + 1]["top"] < arr[j]["top"] and \
                        arr[j + 1]["page_number"] == arr[j]["page_number"]:
                    tmp = arr[j]
                    arr[j] = arr[j + 1]
                    arr[j + 1] = tmp
        return arr
    ...
    def __ocr(self, pagenum, img, chars, ZM=3, device_id: int | None = None):
        ...
        bxs = Recognizer.sort_Y_firstly(
            [
                {"x0": b[0][0] / ZM, "x1": b[1][0] / ZM, "top": b[0][1] / ZM, "text": "", "txt": t,
                 "bottom": b[-1][1] / ZM, "chars": [], "page_number": pagenum}
                for b, t in bxs
        ...
        for b in bxs:
            if not b["text"]:
                ...
                b["box_image"] = self.ocr.get_rotate_crop_image(img_np, np.array(
                    [[left, top], [right, top], [right, bott], [left, bott]], dtype=np.float32))
    ...
    def _line_tag(self, bx, ZM):
        ...
        return "@@{}\t{:.1f}\t{:.1f}\t{:.1f}\t{:.1f}##".format("-".join([str(p) for p in pn]), bx["x0"], bx["x1"], top,
                                                               bott)
            ...
            with sys.modules[LOCK_KEY_pdfplumber]:
                with pdfplumber.open(fnm) if isinstance(fnm, str) else pdfplumber.open(BytesIO(fnm)) as pdf:
                    ...
                    try:
                        ...
                        self.page_chars = [[] for _ in range(
                            page_to - page_from)]  # If failed to extract, using empty list instead.
    ...
    def parse_into_bboxes(self, fnm, callback=None, zoomin=3):
        ...
        def insert_table_figures(tbls_or_figs, layout_type):
            ...
            for (img, txt), poss in tbls_or_figs:
                bboxes = [(i, (b["page_number"], b["x0"], b["x1"], b["top"], b["bottom"])) for i, b in
                          enumerate(self.boxes)]
                dists = [
                    (min_rectangle_distance(
                        (pn, left, right, top + self.page_cum_height[pn], bott + self.page_cum_height[pn]), rect), i)
                    for i, rect in bboxes for pn, left, right, top, bott in poss
                ]
        ...
        for b in self.boxes:
            ...
            b["positions"] = [[pos[0][-1] + 1, *pos[1:]] for pos in
                              RAGFlowPdfParser.extract_positions(b["position_tag"])]
    ...
    def crop(self, text, ZM=3, need_position=False):
        ...
        for ii, (pns, left, right, top, bottom) in enumerate(poss):
            ...
            imgs.append(self.page_images[pns[0]].crop(
