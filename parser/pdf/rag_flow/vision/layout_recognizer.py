...
from parser.pdf.rag_flow.vision.operators import nms
...
class LayoutRecognizer(Recognizer):
    ...
    def __call__(self, image_list, ocr_res, scale_factor=3, thr=0.2, batch_size=16, drop=True):
        ...
        for pn, lts in enumerate(layouts):
            ...
            for lt in ["footer", "header", "reference", "figure caption", "table caption", "title", "table", "text",
                       "figure", "equation"]:
                findLayout(lt)
...
class LayoutRecognizer4YOLOv10(LayoutRecognizer):
    ...
    def postprocess(self, boxes, inputs, thr):
        ...
        return [{"type": self.label_list[class_ids[i]].lower(), "bbox": [float(t) for t in boxes[i].tolist()],
                 "score": float(scores[i])} for i in indices]
